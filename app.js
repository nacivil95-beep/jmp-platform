...
function initMap() {
  ...
  // 드론측량 원본(World File .pgw + 좌표계 .prj)로 정밀 계산한 좌표 (2026-09-17 반영, 9/16 촬영본)
  //   - 원본: 이미지_20cm_260916(저용량).png, 9174×10523px, 픽셀크기 0.2m/px
  //   - 좌표계: EPSG:5186 (한국 중부원점, KGD2002 Central Belt 2010) → WGS84(EPSG:4326)로 변환
  //   - .pgw 좌상단 원점(240232.643927, 465724.964469) 기준, 이미지 크기만큼 우하단 좌표를 계산해
  //     pyproj로 위경도 변환함 (8/19 촬영본과 촬영범위가 미세하게(수 m) 달라 좌표도 소폭 갱신됨)
  const IMAGE_BOUNDS = [
    [36.770254, 127.450751], // 남서(SW) 모서리
    [36.789298, 127.471191]  // 북동(NE) 모서리
  ];
