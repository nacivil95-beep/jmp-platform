...
def extract_quality():
    ...
    if not moisture_file or not plate_file:
        if not moisture_file:
            print("경고: '품질시험' 폴더에서 함수비 엑셀 파일을 찾을 수 없습니다. (파일명에 '함수비' 포함 필요)")
        if not plate_file:
            print("경고: '품질시험' 폴더에서 평판재하시험 엑셀 파일을 찾을 수 없습니다. (파일명에 '평판재하' 포함 필요)")
        print("두 파일이 모두 있어야 갱신합니다. qualityData.js는 변경하지 않고 건너뜁니다.")
        return
